...
do_prepare() {
  ...
  sed -i -r "s|^(\s*)default :ruby_bin, \".+\"|\1default :ruby_bin, \"$(hab pkg path core/ruby27)/bin/ruby\"|" "$HAB_CACHE_SRC_PATH/$pkg_dirname/lib/license_scout/config.rb"

  local _ruby_gems=$(pkg_path_for "core/ruby27")/lib/ruby/gems/2.7.0
  local _pkg_gems="$pkg_prefix/lib"

  export GEM_HOME="${_pkg_gems}/ruby/2.7.0"
...
do_install() {
  ...
  fix_interpreter "$pkg_prefix/bin/gemfile_json" core/ruby27 ruby
}
